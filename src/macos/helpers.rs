    ...
    F: FnOnce(*mut objc2::runtime::AnyObject) -> Result<R>,
{
    ...
    let workspace_class = Class::get("NSWorkspace").ok_or_else(|| {
        anyhow::anyhow!("Failed to get NSWorkspace class")
    })?;
    ...
    for i in 0..count {
        ...
        if let Some(name) = cfstring_to_string(localized_name as *mut c_void) {
...
pub unsafe fn get_ax_string_attr(
    ...
    let text = cfstring_to_string(value)
        .ok_or_else(|| anyhow::anyhow!("Failed to convert attribute '{}' to string", attr_name))?;
...
pub unsafe fn with_app_window<F, R>(
    app_name: &str,
    window_name: &str,
    f: F,
) -> Result<R>
